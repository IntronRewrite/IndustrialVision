...
LastEditTime: 2024-11-24 15:58:58
FilePath: \cloud\2.py
...
#对src聚类
avg_dist = compute_average_distance(pcd_ref)
print(f"点云的平均距离: {avg_dist}")

# 去除pcd_ref的离群点
# 欧式聚类pcd_ref
with o3d.utility.VerbosityContextManager(o3d.utility.VerbosityLevel.Debug) as cm:
    labels = np.array(pcd_ref.cluster_dbscan(eps=avg_dist*4.2, min_points=1, print_progress=True))
...
colors[labels < 0] = 0
pcd_ref.colors = o3d.utility.Vector3dVector(colors[:, :3])
print('欧式聚类成功')
# 可视化聚类结果
...
pcd_ref = pcd_ref.select_by_index(np.where(labels == largest_cluster_idx)[0])
print('保留点最多的一个聚类成功')
# 可视化点云
o3d.visualization.draw_geometries([pcd_ref])
print('点云可视化成功')