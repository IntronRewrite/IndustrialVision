...
int main() {
    ...
    auto pcd_ref_sampled = pcd_ref->UniformDownSample(1000);
    std::cout << "均匀采样src点云成功，采样后点数量: " << pcd_src_sampled->points_.size() << std::endl;
    std::cout << "均匀采样ref点云成功，采样后点数量: " << pcd_ref_sampled->points_.size() << std::endl;
    ...
    // 计算平均距离
    double avg_dist = 0.0;
    open3d::geometry::KDTreeFlann kdtree(*pcd_src_sampled);
    for (size_t i = 0; i < pcd_src_sampled->points_.size(); ++i) {
        std::vector<int> indices(2);
        std::vector<double> distances(2);
        kdtree.SearchKNN(pcd_src_sampled->points_[i], 2, indices, distances);
        avg_dist += std::sqrt(distances[1]);
    }
    avg_dist /= pcd_src_sampled->points_.size();
    ...
    // open3d::visualization::DrawGeometries({neighbor_pcd}, "相邻点结果", 1600, 900);
    ...
    *combined_pcd += *largest_cluster_pcd_sampled;
    ...
    pcd_src->PaintUniformColor(Eigen::Vector3d(0, 1, 0)); // 绿色
    pcd_ref->PaintUniformColor(Eigen::Vector3d(0, 0, 1)); // 红色
