...
void VisualizeRegistration(const open3d::geometry::PointCloud &source,
    ...
    *target_ptr = target;
    ...
    // 设置点云颜色
    source_transformed_ptr->PaintUniformColor(Eigen::Vector3d(1, 0, 0)); // 红色
    target_ptr->PaintUniformColor(Eigen::Vector3d(0, 1, 0)); // 绿色
    // 可视化变换后的源点云和目标点云
...
int main(int argc, char *argv[]) {
    ...
    // 解析体素大小选项，默认为0.05
    float voxel_size =
            utility::GetProgramOptionAsDouble(argc, argv, "--voxel_size", 50);
    // 解析距离乘数选项，默认为1.5
    ...
    if (method == kMethodFeature) {
        ...
        // 将结果保存到txt文件
        std::ofstream outfile("/home/lw/IndustrialVision/build/transformation_matrix.txt", std::ios::trunc);
        if (outfile.is_open()) {
                outfile << registration_result.transformation_ << std::endl;
                outfile.close();
        } else {
